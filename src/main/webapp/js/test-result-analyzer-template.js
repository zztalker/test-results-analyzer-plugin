var tableContent = '<div class="table-row" name = "{{addName text}}" ' +
    ...
    '\n' + '         <div class="table-cell build-result {{applystatus status}}" data-result=\'{{JSON2string this}}\' ' +
                          'title="Build {{buildNumber}}"><a href="{{url}}">{{applyvalue status totalTimeTaken}}</a><br>' +
                          '<span class="failed testresult">{{totalFailed}}</span>'+
                          '<span class="skipped testresult">{{totalSkipped}}</span>'+
                          '<span class="passed testresult">{{totalPassed}}</span></div>' +
    '{{/each}}' +
    '\n' + '</div>' +
    '{{#each children}}\n' +
...
var worstTestsTableBody = '<h2 align="center">Top 10 Most Broken Test</h2>' +
    '\n' + '{{#if this.length}}' +
    '<div class=table>' +
    '\n' + '<div class="heading">' +
